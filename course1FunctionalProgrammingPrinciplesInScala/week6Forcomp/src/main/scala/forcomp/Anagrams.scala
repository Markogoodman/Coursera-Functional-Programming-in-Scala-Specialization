...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.groupBy((c: Char) => c.toLower).map((c, chars) => (c, chars.length())).toList.sortBy((c, _) => c)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    if occurrences.length == 0 then {
      ...
      val ret = for 
        occurrenceIndex <- (0 to occurrences.length-1).toList
        count <- (1 to occurrences(occurrenceIndex)._2).toList
        restOccurrence <- combinations(occurrences.drop(occurrenceIndex+1))
      yield 
        (occurrences(occurrenceIndex)._1, count) :: restOccurrence
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val xMap = x.map(occ => occ._1 -> occ._2).toMap
    
    y.foldLeft(xMap)(
      (m, occ) => 
        (
          if m(occ._1) > occ._2 then {
            m.updated(occ._1, m(occ._1)-occ._2)
          } else {
            m.removed(occ._1)
          }
        )
   ...
   */
  def occurrencesAnagrams(occs: Occurrences): List[Sentence] =
      ...
      val ret =  for 
        ...
        sentence <- occurrencesAnagrams(subtract(occs, wordOccurrences(word)))
      yield 
        word :: sentence
      ret
